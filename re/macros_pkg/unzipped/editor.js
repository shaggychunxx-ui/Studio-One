...
MacroEditor.prototype = new CCL.JS.Component ();
...
{	
	...
	{	
		...
		this.macroList.columns.addColumn (200, JSTRANSLATE ("Command"), CCL.JS.Columns.kTitleID, 50, CCL.JS.Columns.kSizable|CCL.JS.Columns.kCanFit);
		this.macroList.columns.addColumn (300, JSTRANSLATE ("Arguments"), "arguments", 50, CCL.JS.Columns.kSizable|CCL.JS.Columns.kCanFit);
	...
	{
		if(msg.id == CCL.JS.kItemOpened)
		{
			if(subject == this.macroList)
				this.editMacroItem (msg.getArg (0));				
		}
		else if(msg.id == CCL.JS.kCommandSelected)
		{
	...
	this.addCommandToMacro = function (command)
	{
		...
		if(command)
		{
			...
			if(arguments.length > 0 && arguments != "...")
			{
				...
				for(let i in args)
				{
					let argName = args[i].trim ();
					element.arguments.push (new CommandArgument (argName, ""));
				}
			}
	...
	this.moveMacroItems = function (offset)
	{
		...
		if(position < 0)
			position = 0;
		else if(position >= this.macroList.itemCount)
			position = this.macroList.itemCount - 1;
	...
	this.editMacroItem = function (macroItem)
	{
		...
		if(1) // EXPERIMENTAL: use edit task window for argument editing
		{
			...
			if(command && command.arguments == "..." && command.classID.length > 0)
			{
				...
				if(argumentEditor.runDialog (args, command.classID) != Host.Results.kResultOk)
					return;
