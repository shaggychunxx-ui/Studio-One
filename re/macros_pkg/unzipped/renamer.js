...
function EventRenamer ()
{
	...
	this.startup = function ()
	{
		if(!this.initialized)
		{
			...
			if(!Host.IO.File (path).exists ())
				Host.IO.File (CCL.JS.ResourceUrl ("eventnames.txt")).copyTo (path);
	...
	this.loadStrings = function (path)
	{
		...
		{
			var line = textFile.readLine ().trim ();
			if(line.length == 0)
				continue;
	...
	this.buildMenu = function (menu, commandHandler)
	{
		for(let i in this.bundles)
		{
			...
			for(let j in bundle.strings)
			{
				...
				if(string.charAt (0) == "-")
				{
					let header = string.substr (1).trim ();
					if(header.length == 0)
		...
		menu.addCommandItem (JSTRANSLATE ("Edit Names"), kMacrosCategory, "Edit Names", 0);
		menu.addCommandItem (JSTRANSLATE ("Reload Names"), kMacrosCategory, "Reload Names", 0);
	}
}
...
EventNameEditor.prototype = new CCL.JS.Component ();
...
{	
	...
	this.loadFromFile = function (path)
	{
		...
		if(textFile)
		{
			let nl = CCL.JS.EndLine (); // use platform line ending
			while(!textFile.endOfStream)
