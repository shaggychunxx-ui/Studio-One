...
const kPagesCategory = "Pages";

const kMacrosSignal = "studioonemacros";
const kMacrosRescanned = "MacrosRescanned";
const kMacroRenamed = "MacroRenamed";
const kImportMacroPage = "ImportMacroPage";
...
var theMacroPageFileType = {description: JSTRANSLATE ("Studio One Macro Page"), extension: "macropage", mimetype: "application/x-presonus-studioonemacropage"};
...
Macro.prototype.loadFromFile = function (path)
{
	...
	{
		var msg = 'Macro "' + path.name.substring(0, path.name.lastIndexOf('.')) + '": ' + xmlTree.errorMessage;
		Host.Console.writeLine (msg);
		return false;
	}
...
MacroExecuter = function (macro, defaultHandler)
{
	...
	{
		Host.GUI.Commands.beginTransaction (macro.title)
	...
	{
		let element = macro.elements[this.currentIndex];
		if(!element)
			return;

		switch(element.type)
		{
		case kCommandElement :
			this.onCommand (element);
			break;
		}

		this.currentIndex++;
		if(this.currentIndex < macro.elements.length)
		{
			// defer execution of next command so that deferred actions (triggered by the current command) can happen before
			Host.Signals.postMessage (this, -1, "nextCommand");
		}
		else
